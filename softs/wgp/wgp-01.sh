...
EOF
  ...
  proot-distro login --no-kill-on-exit zpd-wgp-01 -- /bin/bash -c "cd /root/wgp01 && ./wgp_start& sleep 30; exit;"
...
if [ "$CMD" == "stop" ]; then
  proot-distro login --no-kill-on-exit zpd-wgp-01 -- /bin/bash -c "cd /root/wgp01 && ./wgp_stop& sleep 30; exit;"
  exit
fi
